...
/******************************************************************************
 * @file     SYSTM002.c
 * @brief    SCU_SystemTimer_SYSTM002 App implementation file. This App provides
...
SYSTM002_TimerObject*  TimerList = 0;
...
static void  SYSTM002_lTimerHandler (void)
{
  ...
  {	
    ...
    if(TmrObjPtr->TimerType == SYSTM002_ONE_SHOT)
    {
    ...
    else if(TmrObjPtr->TimerType == SYSTM002_PERIODIC)
    {
      /* Yes,remove this timer from timer list */
      SYSTM002_lRemoveTimerList(TmrObjPtr->TimerID);
      /* Reset timer tick             */
...
{ 
  ...
  if(TmrObjPtr == NULL)
  {
    ...
    DBG002_ERROR(APP_GID,SYSTM002_INVALID_HANDLE_ERROR, 0, NULL);
  }
  else
  {
...
{
    uint32_t Status = 0UL;
  DBG002_FUNCTION_ENTRY(APP_GID,SYSTM002_FUNCTION_ENTRY);
   /* <<<DD_SYSTM002 _API_1>>> */
  /** Initialize the header of the list */
  TimerList = NULL;
  /**   Initialize timer tracker  */
  Status = SysTick_Config((uint32_t)(SYSTM002_SYSTICK_INTERVAL * SYSTM002_SYS_CORE_CLOCK));
  if(Status == 1U)
  {
        DBG002_ERROR(APP_GID,SYSTM002_ERROR, 0, NULL);
  }
    NVIC_SetPriority(SysTick_IRQn, 1);
  TimerTracker = 0UL;
  DBG002_FUNCTION_EXIT(APP_GID,SYSTM002_FUNCTION_EXIT);
}
...
)
{
  ...
  /* Check for input parameter */
    if((TimerType != SYSTM002_ONE_SHOT) && (TimerType != SYSTM002_PERIODIC))
    {
      DBG002_ERROR(GID_SYSTM002,SYSTM002_INVALID_HANDLE_ERROR, 0, NULL);
      Error=(uint32_t)1UL;
    }
    ...
    if (!Error)	
    {
       for(Count = 0UL; Count < SYSTM002_CFG_MAX_TMR; Count++)
       {
           ...
           if((TimerTracker & ((uint32_t)1U << Count)) == 0U)
           {
             ...
             TimerTbl[Count].TimerCount  = ((Period / SYSTM002_SYSTICK_INTERVAL)\
                                                    +HW_TIMER_ADDITIONAL_CNT);
               TimerTbl[Count].TimerReload	= (Period / SYSTM002_SYSTICK_INTERVAL);
...
status_t SYSTM002_StartTimer(handle_t  Handle) 
{
  ...
  if(Error == (status_t)DAVEApp_SUCCESS)
  {
    ...
    if(TimerTbl[(Handle - 1U)].TimerState != SYSTM002_STATE_RUNNING)
    {
...
status_t SYSTM002_StopTimer(handle_t Handle) 
{
  ...
  if(Error == (status_t)DAVEApp_SUCCESS)
  {
    ...
    if(TimerTbl[(Handle - 1U)].TimerState != SYSTM002_STATE_STOPPED)
    {
...
{
  /* <<<DD_SYSTM002 _API_6>>> */
  return CONVERT_SYSTICK_COUNT_TO_USEC(SysTickCount);
}
...
{
  /* <<<DD_SYSTM002 _API_7>>> */
  uint32_t	Count  = CONVERT_USEC_TO_SYSICK_COUNT(Period);
  return Count;
}
